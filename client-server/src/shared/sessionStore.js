...
const EXPIRED_SESSION_RETENTION_MS = 5 * 60 * 1000;
...
function cleanupExpiredSessions() {
  ...
  sessions.forEach((session, sessionId) => {
    ...
    const expiredAtMs = new Date(expiresAt).getTime();
    if (expiredAtMs > nowMs) {
      return;
    }

    // Keep expired sessions briefly so APIs can return deterministic TTL failures.
    if (nowMs - expiredAtMs < EXPIRED_SESSION_RETENTION_MS) {
      return;
    }
